// необ€зательный модуль отображени€ списка активных юзеров
// требует наличи€ контейнера usersdiv, jquery, переменных apihost и authorized
// возможна зависимость от настроек определЄнных в chat.js
...
function DrawSubList(list) {
	...
	{	if(key=="count") continue;
		...
		// когда-нибудь в list[key] будут какие-то параметры пользовател€, а пока можем знать только, что он онлайн
	};
	$('#usersdiv').append("<p>&nbsp;</p>");
};
...
function DrawUserList(u) {
	...
	if(u.topic) { //upd от 21.07.2017
		...
		document.title = "јльфа-хаб: " + u.topic;
	};
	return;
}
...
function UpdateUsers() {
	...
	//.complete(function() { console.log("«авершение выполнени€"); }); // аналог then
	;
}